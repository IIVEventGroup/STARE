...
def convert_event_img_aedat(events, style, height=260, width=346):
    """
    ...
    if style == 'VisEvent': 
        ...
        voxel_pos, voxel_neg = events_to_neg_pos_voxel_torch(xs, ys, ts, ps, B, device, (height, width), temporal_bilinear)
        ...
        # sensor_size = (260,346)
        temporal_bilinear = True
        voxel_complex = events_to_voxel_torch(xs, ys, ts, ps, B, device, (height, width), temporal_bilinear)
        ...
        """ pending"""
        dvs_img = torch.zeros((height, width), dtype=np.uint8)
