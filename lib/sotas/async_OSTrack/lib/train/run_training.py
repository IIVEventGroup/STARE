...
def run_training(script_name, config_name, cudnn_benchmark=True, local_rank=-1, save_dir=None, base_seed=None,
                 ...
                 distill=None, script_teacher=None, config_teacher=None, use_async=None):
    ...
    if use_async:
        settings.use_async = use_async
        expr_func = getattr(expr_module, 'run_async')
    else:
...
def main():
    ...
    # for async training
    parser.add_argument('--use_async', type=int, choices=[0, 1], default=0)
