...
from lib.train.actors import OSTrackActor, AsyncOSTrackActor
# for import modules
import importlib
...
def run_async(settings):
    settings.description = 'Training script for Async OSTrack'
    ...
    # Build dataloaders
    loader_train, loader_val = build_async_dataloaders(cfg, settings)
    ...
    if settings.script_name == "ostrack":
        ...
        loss_weight = {
            ...
            'focal': 1.,
            'cls': 1.0,
            'feat_hard': cfg.TRAIN.FEAT_HARD_WEIGHT,
            'feat_soft': cfg.TRAIN.FEAT_SOFT_WEIGHT,
        }
        actor = AsyncOSTrackActor(
            net=net,
            objective=objective,
