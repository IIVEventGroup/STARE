...
class AsyncTrackingSampler(torch.utils.data.Dataset):
    """ Class responsible for sampling frames from training sequences to form batches.
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 num_search_frames, num_template_frames=1, processing=no_processing, frame_sample_mode='causal',
                 train_cls=False, pos_prob=0.5, async_horizon=4):
        """
        args:
            datasets - List of datasets to be used for training
            p_datasets - List containing the probabilities by which each dataset will be sampled
        ...
        self.processing = processing
        self.frame_sample_mode = frame_sample_mode
        self.async_horizon = async_horizon
        ...
        while not valid:
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    while search_frame_ids is None:
                        base_frame_id = self._sample_visible_ids(
                            ...
                            num_ids=1,
                            min_id=self.num_template_frames - 1,
                            max_id=len(visible) - self.num_search_frames - self.async_horizon - 2
                        )
                        ...
                        template_frame_ids = base_frame_id + prev_frame_ids
                        max_search_id = min(template_frame_ids[0] + self.max_gap + gap_increase, len(visible)) - self.async_horizon - 1
                        ...
                        for i in range(self.async_horizon):
                            search_frame_ids.append(search_frame_ids[0] + i + 1)
            ...
            try:
                ...
                template_masks = template_anno['mask'] if 'mask' in template_anno else [torch.zeros(
                    (H, W))] * self.num_template_frames
                search_masks = search_anno['mask'] if 'mask' in search_anno else [torch.zeros(
                    (H, W))] * (self.num_search_frames + self.async_horizon)
