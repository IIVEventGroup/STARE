...
# datasets related
from lib.train.dataset import Lasot, Got10k, MSCOCOSeq, ImagenetVID, TrackingNet, FE240, VisEvent, ESOT500, ESOT500H
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        ...
        #                 "COCO17", "VID", "TRACKINGNET","FE240","FE240_240","FE240_val","VisEvent","VisEvent_val","VisEvent_val_stnet",
        #                 "ESOT500","ESOT250","ESOT125","ESOT050","ESOT020","ESOT500_val","ESOT500H","ESOT500H_val"]
        ...
        if name == "VisEvent_val_stnet":
            print("Building VisEvent_val_stnet dataset")
...
def build_async_dataloaders(cfg, settings):
    # Data transform
    transform_joint = tfm.Transform(tfm.RandomHorizontalFlip(probability=0.5))
    ...
    dataset_train = sampler.AsyncTrackingSampler(
        ...
        train_cls=train_cls,
        async_horizon=cfg.DATA.ASYNC_HORIZON
    )
    train_sampler = DistributedSampler(dataset_train) if settings.local_rank != -1 else None
    shuffle = False if settings.local_rank != -1 else True
    ...
    # Validation samplers and loaders
    dataset_val = sampler.AsyncTrackingSampler(
