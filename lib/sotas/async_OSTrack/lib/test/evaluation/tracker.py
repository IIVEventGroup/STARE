...
class Tracker:
    ...
    def run_sequence(self, seq, stream_setting=None, debug=None, visualization=None, visdom_info=None):
        ...
        ###
        # params.checkpoint = params.checkpoint.replace('ep0150', f'ep{self.run_id:04d}')
        ###
...
#########################################################################################
        ...
        if not stream_setting.convert_time:
            ...
            while 1:
                ...
                tracker.async_order += 1

                ##################################################################################################

                # if active_state['is_last_activated'] or tracker.async_order % 3 != 1 or tracker.async_order == 1:
                ...
                if stream_setting.sim:
                    ...
                    if tracker.async_order % 3 > 0:
                        sim_runtime = stream_setting.sim_runtime.get(
                            self.name + '_' + self.parameter_name + '_fast',
                            stream_setting.sim_runtime_rt,
                        )

                    else:
                        sim_runtime = stream_setting.sim_runtime.get(
                            self.name + '_' + self.parameter_name + '_fast_slow_diff',
                            stream_setting.sim_runtime_rt,
                        )
                    ##################################################################################################
