...
class EventCARLA(BaseVideoDataset):
    """ EventCARLA dataset.
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        """
        root = env_settings().eventcarla_dir if root is None else root
        super().__init__('EventCARLA', root, image_loader)
    ...
    def get_sequences_in_class(self, class_name):
        raise('EventCARLA does not support get sequences in class')

    def _get_sequence_list(self, split):
        seq_list = []
        with open ('data/EventSOT/EventCARLA/eventCarla_{}.txt'.format(split),'r') as f:
            for line in f:
...
if __name__ == '__main__':
    dataset = EventCARLA(root='data/EventSOT/EventCARLA/VoxelGrid',split='test')
