import numpy as np
from lib.test.evaluation.data import Sequence, SequenceRGBE, BaseDataset, SequenceList
...
class ESOT500HDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_name):
        ...
        return Sequence(sequence_name + '_{}_w{}ms'.format(self.fps, self.window), frames_list, 'esoth_{}_w{}ms'.format(self.fps, self.window), ground_truth_rect.reshape(-1, 4))
    ...
    def _get_sequence_list(self, split):
        ...
        seq_list = []
        with open (self.env_settings.esot500h_dir + '/{}.txt'.format(split),'r') as f:
            for line in f:
...
class ESOT500Dataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_name):
        ...
        anno_file = self.env_settings.esot500_dir + '/anno_t/{}.txt'.format(sequence_name)
        t_anno = np.loadtxt(anno_file, delimiter=' ')
        aeFile = os.path.join(self.env_settings.esot500_dir, 'aedat4', sequence_name + '.aedat4')
        rgb_frame_info = {
            'aeFile': aeFile,
            'fps': self.fps,
            'window': self.window,
        ...
        return SequenceRGBE(
            sequence_name+'_{}_w{}ms'.format(self.fps, self.window),
            frames_list,
            'esot_{}_w{}ms'.format(self.fps,self.window),
            ground_truth_rect.reshape(-1, 4),
            rgb_frame_info=rgb_frame_info,
        )
