...
class Preprocessor(object):
    ...
    #     # img_tensor_norm = ((img_tensor / 255.0) - self.mean) / self.std  # (1,3,H,W) # ! No Norm used
...
class PreprocessorX(object):
    ...
    def process(self, img_arr: np.ndarray, amask_arr: np.ndarray):
        # Deal with the image patch
        img_tensor = torch.tensor(img_arr).cuda().float().permute((2,0,1)).unsqueeze(dim=0)
        # img_tensor_norm = ((img_tensor / 255.0) - torch.mean(img_tensor,dim=(0,2,3),keepdim=True)) / torch.std(img_tensor,dim=(0,2,3),keepdim=True)  # (1,3,H,W)
        img_tensor_norm = self._normalizeVoxelGrid(img_tensor)
        # Deal with the attention mask
        amask_tensor = torch.from_numpy(amask_arr).to(torch.bool).cuda().unsqueeze(dim=0)  # (1,H,W)
        return NestedTensor(img_tensor_norm, amask_tensor)

    def _normalizeVoxelGrid(self, voxel_grid: torch.Tensor):
