...
def trackerlist(name: str, parameter_name: str, dataset_name: str, run_ids = None, pred_next: int = 0, display_name: str = None,
    ...
    if run_ids is None or isinstance(run_ids, int):
        run_ids = [run_ids]
    return [Tracker(name, parameter_name, dataset_name, run_id, pred_next, display_name=display_name, result_only=result_only) for run_id in run_ids]
...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, dataset_name: str, run_id: int = None, pred_next: int = 0, use_cas: bool = False, display_name: str = None,
        ...
        if self.run_id is None:
            ...
            self.segmentation_dir = '{}/{}/{}'.format(self.env.segmentation_path, self.name, self.parameter_name)
        else:
    ...
    def create_tracker(self, params):
        tracker = self.tracker_class(params, self.env, self.dataset_name)
        tracker.visdom = self.visdom
        return tracker

    def run_sequence(self, seq, stream_setting=None, pred_next=0, debug=None, visualization=None, visdom_info=None):
    ...
    def _track_evstream(self, tracker, seq, init_info, stream_setting, pred_next, **kwargs):
        """
        Core Function
        stream_setting:
