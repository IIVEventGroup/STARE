...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pred_next=0, pos_prob=0.5):
        """
        args:
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None,
        ...
        if min_id is None or min_id < 0:
            min_id = 0
        if not self.pred_next:
            if max_id is None or max_id > len(visible):
                max_id = len(visible)
        else:
            if max_id is None or max_id > len(visible)-1:
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            try:
                ...
                if not self.pred_next:
                    pred_next_frames, pred_next_anno, meta_obj_pred_next = None,None,None
                    time_to_next = None
                    pred_next_mask = None
                else:
                    pred_next_frame_ids = [search_frame_ids[0]+1]
                    pred_next_frames, pred_next_anno, meta_obj_pred_next = dataset.get_frames(seq_id, pred_next_frame_ids, seq_info_dict)
                    time_to_next = dataset.time_gap
                    pred_next_masks = pred_next_anno['mask'] if 'mask' in pred_next_anno else [torch.zeros((H, W))] * self.num_search_frames
                ...
                data = TensorDict({'template_images': template_frames,
                                   ...
                                   'pred_next_images': pred_next_frames,
                                   'pred_next_anno': pred_next_anno['bbox'] if pred_next_anno else None,
                                   'pred_next_masks': pred_next_masks,
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset):
        ...
        while not enough_visible_frames:
            ...
            # Sample frames
            seq_info_dict = dataset.get_sequence_info(seq_id) #250->bbox[3260,4]
