    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            if self.settings.pred_next:
                data['pred_next_images'], data['pred_next_anno'], data['pred_next_masks'] = self.transform['joint'](
                    image=data['pred_next_images'], bbox=data['pred_next_anno'], mask=data['pred_next_masks'], new_roll=False)
        ...
        if self.settings.pred_next:
            # Crop image region centered at jittered_anno box; for pred_next, use the search_area_factor and transform of search
            jittered_anno = [self._get_jittered_box(a, 'search') for a in data['pred_next_anno']]
            crops, boxes, att_mask, mask_crops = prutils.jittered_center_crop(data['pred_next_images'], jittered_anno,
                                                                              data['pred_next_anno'], self.search_area_factor['search'],
                                                                              self.output_sz['search'], masks=data['pred_next_masks'])
            data['pred_next_images'], data['pred_next_anno'], data['pred_next_att'], data['pred_next_masks'] = self.transform['search'](
                image=crops, bbox=boxes, att=att_mask, mask=mask_crops, joint=False)
        ...
        if self.mode == 'sequence':
            data = data.apply(stack_tensors) #[1,3,128,128]
        else:
