...
from ltr.dataset import LasotCandidateMatching, ESOT500, FE240
...
def run(settings):
    ...
    # Train datasets
    fe240_train = FE240(settings.env.fe240_dir, split='train')
    ...
    dataset_train = sampler.SequentialTargetCandidateMatchingSampler(fe240_train,
    ...
    dataset_val = sampler.SequentialTargetCandidateMatchingSampler(fe240_val,
                                                                  samples_per_epoch=int(settings.batch_size * 10),
                                                                  processing=processing_val, sup_modes=['self_sup'],
                                                                  frame_modes=['G', 'H', 'J', 'K'],
                                                                  p_frame_modes=[0.2, 0.4, 0.2, 0.2])

    dataset_val_realHH = sampler.SequentialTargetCandidateMatchingSampler(fe240_val,
                                                                          samples_per_epoch=int(settings.batch_size * 5),
                                                                          processing=processing_val_real, sup_modes=['partial_sup'],
                                                                          subseq_modes=['HH'],
                                                                          p_subseq_modes=[1.])

    dataset_val_realHK = sampler.SequentialTargetCandidateMatchingSampler(fe240_val,
