...
class SequenceEvent:
    """Class for the sequence in an evaluation."""
    def __init__(self, name, frames, events, dataset, ground_truth_rect, ground_truth_t, ground_truth_seg=None, init_data=None,
                 object_class=None, target_visible=None, object_ids=None, multiobj_mode=False):
        self.name = name
        self.frames = frames
        self.events = events
        ...
        self.multiobj_mode = multiobj_mode
        self.init_data = self._construct_init_data(init_data)
        self.init_data_t = self._construct_init_data_t(init_data)
        self._ensure_start_frame()
    ...
    def frame_info_t(self, timestamp):
        info = self.object_init_data_t(frame_num=timestamp) # TODO
